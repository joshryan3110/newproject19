...
function preload(){
    ...
    smolTrexImg=loadImage("trex1.png")
...
function draw() {
    ...
    console.log(trex.y)
    textSize(30)
    text("Distance: "+distance,1120,100)
    textSize(30)
    text("Babies Collected: "+amount,1020,200)
    ...
    if(gamestate==="start"){
        ...
        ground.velocityX=-(6+distance/100);
    ...
    if(gamestate==="end"){
        ...
        projectGroup.setLifetimeEach(-1);
        
        smolTrexGroup.setVelocityXEach(0)
        smolTrexGroup.setLifetimeEach(-1)
...
function spawnCloud(){
  ...
  if(frameCount%45===0){
    ...
    cloud.velocityX=-(5+distance/100);
...
function spawnCactus(){
  ...
  if(frameCount%70===0){
    ...
    cactuss.scale=0.7
    cactuss.velocityX=-(6+distance/100);
...
function spawnDaBaby(){
  if(frameCount%230===0){
    smolTrex=createSprite(1360,390)
    smolTrexGroup.add(smolTrex);
    smolTrex.addImage(smolTrexImg);
    smolTrex.scale=0.5
    smolTrex.velocityX=-(6+distance/100);
    smolTrex.lifetime=300;
    // smolTrex.debug=true
    smolTrex.setCollider("circle",0,0,30)
  }
}
